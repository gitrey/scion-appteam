...
2. **Create tasks** — Use `TaskCreate` to create work items for the pipeline:
   ...
   - **Task N+3:** devops — Build containers, provision infra, and deploy to GCP
   - **Task N+4:** Reviewer — Code review (if reviewer agent exists)
...
5. **Follow the mandatory pipeline:**
   - PO feedback → PM creates spec → TPM creates backlog → SWEs implement → SWE-Test verifies code, ui-test verifies visual UI & perf-test profiles load/latency → Reviewer reviews → devops deploys to GCP
