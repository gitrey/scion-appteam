...
5. **Follow the mandatory pipeline:**
   - PO feedback → PM creates spec → TPM creates backlog → SWEs implement → QA Triad verifies (SWE-Test code, ui-test visual, perf-test load, secops security) → Reviewer reviews & doc updates api → devops deploys to GCP → sre probes health & alerts
