---
name: regenerate
description: Regenerate project files from settings
---
...
User invokes `/regenerate` to regenerate GEMINI.md and all agent/skill files from saved settings.
...
2. **Run regeneration** — Execute:
   ...
   appteam -r
   ```
   This reads `.appteam/settings.json` and regenerates all files (GEMINI.md, agent definitions, skill files, docs templates)
